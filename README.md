...
| `sounds-enabled` | Lobby / countdown / Wither sounds | `true` |
| `damage-indicator-enabled` | Action-bar "who lost hearts" indicator | `true` |
...
| `/hc testdamage [hearts]` | Test the damage indicator (real damage if in-game, else a simulated action bar) — *admin* |
...
**Damage indicator.** Whenever a player loses health, an action bar message
(above the XP bar) shows for ~5s *who* lost how many hearts and the source
(attacking mob/player or cause), so the team sees where the shared-life
drain comes from. Toggle with `damage-indicator-enabled`. Test it with
`/hc testdamage [hearts]`.

**Sounds.** Configurable via `sounds-enabled`. Lobby arrival plays a
bell/orb chime; the countdown ticks with a rising pitch (sharper in the
last 3s); a **Wither spawn roar** marks the start of a new run and a
**Wither death** marks game over.
